...
## Batch 3 — parallel tracks A–D, all merged (134 tests, coverage ~82%)

| Track | Landed |
|---|---|
| A pipeline | item 9 (analyze-now refuses excluded, closes client → 400), item 11 (cooldown keyed on container+classification), rejected-issue suppression (24h), `alert_min_classification`, `event_retention_days` + daily prune job, nightly briefing pushed to Telegram, migration 0006 |
| B buffer/watcher | item 12 (LogBuffer lock, incremental tokens, idle flush from health tick, stack-trace continuation), item 13 (per-generation stop events, thread joins, idempotent start), default keywords → 15 incl. `traceback,failed,denied,killed,oom,unhealthy,segfault,out of memory` (phrase-aware prefilter) |
| C lifecycle | functionality gap #1: die/oom/kill/restart/unhealthy → `container_event` rows (no LLM), restart-storm alert (`restart_alert_count` in `restart_alert_window_minutes`, cooldown), briefing uses real restart counts, migration 0007 |
| D prompts/UX | gaps 5–8: rubric + one-shot in SENTINEL_ANALYSIS (auto-refreshed on existing DBs when still default), `response_format: json_object` w/ fallback, alert card log excerpt + dashboard hint, reports render markdown (safe converter) + real status badges, events page status filter/sticky/issue links/alert_error, "Sentinel is stopped" banner, save-then-test JS, `extra_hosts: host-gateway` in compose, README fixes, GitHub Actions CI |
| merge | conflicts in `docker_watcher.py`, `schemas/settings.py`, `briefing.py` hand-resolved (union); alembic chain verified: fresh DB → single head `b2c3d4e5f6a7`; simulated legacy DB → stamp 0002 → 5 upgrades apply. Settings UI got the 4 new fields; dashboard shows analyze errors. |

**Second-opinion (Qwen) on tracks A–D:** A: 0/8 claims valid (`server_default="14"` is the standard idiom; `Settings.singleton()` is a per-call `session.get`). B: 0 confirmed defects, 1 hardening taken (idempotent `start()`). C: 1/6 valid (O(N·M) scan in `_restart_evidence` → single pass); rejected "batch add NOT NULL fails on SQLite" (0002 does it; verified by running the chain). D: 1/4 valid (dashboard "Skipped" bucket swallowed `queued`/`container_event`); rejected "bootstrap overwrites customised prompts" (`is_default` is cleared on edit at `api/prompts.py:27`) and "retry loop double-posts" (single fallback retry). Net: the local model is a decent nudge (~1 real finding per review) but ~75% of its "critical" claims were wrong on verification.
